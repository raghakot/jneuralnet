...
package ui.builder.network;

import custom.ActivationCellEditor;
import custom.ActivationComboRenderer;
 ...
 * @author Ragha
 */
public class NetworkBuilder extends javax.swing.JPanel
{
    ...
    {        
        ...
        for(AbstractActivation act : cfg.getMapActivations()
                .getUnWrappedPluginList(AbstractActivation.class, false))
            cbxActivationFunctions.addItem(act);
        
        tblNetworkDesign.getColumnModel().getColumn(2)
                .setCellEditor(new ActivationCellEditor(cbxActivationFunctions));
    }
    ...
    private void initComponents() {
        ...
        tblNetworkDesign.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {"Output", new Integer(2), new Sigmoid().getName()}
            },
            new String [] {
    ...
    private void btnCreateNetActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCreateNetActionPerformed
        try
        {
            ...
            String act = tblNetworkDesign.getValueAt(0, 2).toString();            
            nn.getOutputLayer().setActivationFunction(getActivation(act));
            ...
            for(int i=0;i<hiddenLayerIndex;i++)
            {
                ...
                act = tblNetworkDesign.getValueAt(i + 1, 2).toString();
                hiddenLayer.setActivationFunction(getActivation(act));
            ...
            MainWindow.getInstance().getStatusBar().showMessage(
                    "Neural Network is refreshed with "
                    +" Inputs = "+numInputs + ","
                    +" Outputs = "+numOutputs
                    +" and Hidden Layers = "+hiddenLayerIndex, Color.BLACK);
        } 
        catch(Exception e)
        {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Action failed due to '"
                    + e.getMessage() + "'",
    ...
    private AbstractActivation getActivation(String key)
    {
        return Configuration.getInstance().getMapActivations()
                .getUnWrappedPluginMap(AbstractActivation.class, false)
                .get(key);
    }
